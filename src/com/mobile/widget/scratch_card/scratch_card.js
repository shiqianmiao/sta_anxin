 ...
 *     data-widget="com/mobile/widget/scratch_card/test/test.js#lottery"
...
module.exports = Widget.define({
    ...
    init: function(config) {
        ...
        this.config.canvas = this.config.$el[0];
    ...
    drawStart: function(e) {
        ...
        var funcName = config.$el.data('getImage');
    ...
    drawEnd: function(e) {
        ...
        if (!config.resultFlag) {
            if (j <= particle * 0.1) {
                var funcName = config.$el.data('callback');
    ...
    draw: function(e) {
        ...
        if (config.drawFlag) {
            ...
            if (touches) {
                e = touches[touches.length - 1];
            }

            var x = (e.clientX + document.body.scrollLeft || e.pageX) - config.offsetX || 0;
            var y = (e.clientY + document.body.scrollTop || e.pageY) - config.offsetY || 0;
            var ctx = config.ctx;
