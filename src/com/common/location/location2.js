...
var $ = require('jquery'),
    ...
    LEvent = require('./location_event2.js'),
    Field;
var DISPLAY_URL = {
...
var typeInfo = {
    ...
    city: {
        district: LEvent.getDsitrictByCityId
    },
    district: {
        street: LEvent.getStreetByDsitrictId
    }
};
    ...
    _decide: function() {
        ...
        $els.each(function(i, el) {
            var $el = $(el),
                ...
                defTitle = data.locationDefault || $el.data('default') || '请选择',
    ...
    _initDisplay: function() {
        ...
        $.each(groups, function(i, group) {
            ...
            $.each(group, function(j, field) {
                ...
                //ie6下渲染问题，会导致长度不够文字显示不全，这里进行重置
                setTimeout(function() {
                    var width = field.$el[0].style.width;
                    field.$el.css('width', 'auto');
                    field.$el.css('width', width);
                }, 100);
            });
        });
    },
